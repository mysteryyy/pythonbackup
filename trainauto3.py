...
shape=(len(a)-(self.lb-1), self.lb, a.shape[1]), strides=(s0,s0,s1))
    ...
    def mlplstm(self,x,y,yh,yd,x1,y1,yh1,yd1,ys,ys1,ch):
        def modelz():
            model = Sequential()
            model.add(TimeDistributed(Dense(15), input_shape=(20, 4)))
            ...
            history = model.fit(x,y, epochs =100 ,verbose = 1,batch_size= 350,shuffle=True)
